...
# exercises the installed `trunk-cli` on throwaway repositories: help, version,
# `trunk-cli docs` with and without a terminal, and the generic clone and init
...
CLI="$WORK/consumer/node_modules/.bin/trunk-cli"
[ -x "$CLI" ] || fail "installed package has no trunk executable"

step "trunk-cli --help lists only clone, init and docs"
HELP=$("$CLI" --help)
COMMANDS=$(printf '%s\n' "$HELP" | grep -E '^ *\$ trunk-cli ' | awk '{print $3}' | tr '\n' ' ')
[ "$COMMANDS" = "clone init docs " ] || fail "help lists: $COMMANDS"
if printf '%s\n' "$HELP" | grep -Eiq 'trunk(-cli)? new|--pm|--server|--caddy|--remote|--owner|--public'; then
  fail "help still mentions a removed command or flag"
fi

step "trunk-cli --version matches package.json"
EXPECTED=$(node -p "require('$ROOT/package.json').version")
[ "$("$CLI" --version)" = "$EXPECTED" ] || fail "version differs from $EXPECTED"

step "trunk-cli docs without a terminal exits 2"
CODE=0
"$CLI" docs </dev/null >"$WORK/docs.out" 2>"$WORK/docs.err" || CODE=$?
[ "$CODE" = 2 ] || fail "trunk-cli docs exited $CODE without a terminal"
grep -q "needs a terminal" "$WORK/docs.err" || fail "trunk-cli docs gave no guidance"
if grep -Eq 'Raw mode|component:|^ +at ' "$WORK/docs.err"; then
  fail "trunk-cli docs printed a stack trace"
fi

step "trunk-cli docs shows the two-topic menu in a real terminal"
...
if tmux -L "$SOCKET" has-session -t docs 2>/dev/null; then
  fail "trunk-cli docs did not exit after q"
fi
...
step "trunk-cli clone --yes sets up a generic project"
"$CLI" clone --yes "file://$WORK/remote.git" "$WORK/cloned" >"$WORK/clone.out" 2>&1 \
  || { cat "$WORK/clone.out"; fail "trunk-cli clone failed"; }
...
step "trunk-cli init --yes sets up a bare project that has no origin"
...
"$CLI" init --yes "$WORK/originless" >"$WORK/init.out" 2>&1 \
  || { cat "$WORK/init.out"; fail "trunk-cli init failed without an origin"; }
...
step "trunk-cli init --yes keeps an existing config byte for byte"
mkdir "$WORK/existing"
...
"$CLI" init --yes "$WORK/existing" >"$WORK/keep.out" 2>&1 \
  || { cat "$WORK/keep.out"; fail "trunk-cli init failed on an existing config"; }
cmp -s "$WORK/existing/main/.config/wt.toml" "$WORK/expected.toml" \
  || fail "trunk-cli init --yes changed an existing config"

step "trunk-cli init refuses an empty bare repository without creating history"
mkdir "$WORK/empty"
git init --quiet --bare "$WORK/empty/.git"
CODE=0
"$CLI" init --yes "$WORK/empty" >"$WORK/empty.out" 2>&1 || CODE=$?
[ "$CODE" = 2 ] || fail "trunk-cli init exited $CODE for an empty repository"
grep -q "empty bare repository" "$WORK/empty.out" || fail "no explanation for the empty repository"
[ -z "$(git --git-dir "$WORK/empty/.git" for-each-ref)" ] || fail "trunk-cli init created history"
